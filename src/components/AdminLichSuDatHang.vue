    ...
    <table class="table">
      ...
      <tbody>
        <template v-for="dt in data" :key="dt.maHoaDon">
          <tr
            v-for="(sp, index) in dt.chiTietHoaDon"
            :key="`${dt.maHoaDon}-${sp.maSanPham}`"
          >
            <template v-if="index === 0">
              <td :rowspan="dt.chiTietHoaDon.length">{{ dt.maHoaDon }}</td>
              <td :rowspan="dt.chiTietHoaDon.length">
                {{ dt.khachHang ? dt.khachHang.tenKhachHang : "" }}
              </td>
              <td :rowspan="dt.chiTietHoaDon.length">
                {{ dt.khachHang ? dt.khachHang.sdt : "" }}
              </td>
...
onMounted(() => {
  ...
  axios
    .get("http://localhost:8080/admin/lich-su-dat-hang", {
      headers: {
